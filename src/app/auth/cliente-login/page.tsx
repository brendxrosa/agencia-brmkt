...
export default function ClienteLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    // Redirect simples — sem verificar role aqui
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center bg-offwhite p-8">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            <span className="inline-block bg-rosa/10 text-rosa text-xs font-semibold px-3 py-1 rounded-full">
              🧑‍💼 Portal do Cliente
            </span>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input className="input" type="email" value={email}
                onChange={e => setEmail(e.target.value)} required autoFocus placeholder="seu@email.com" />
            </div>
            <div>
          ...
          <p className="text-center text-xs text-gray-400 mt-6">
            Sou da equipe →{' '}
            <Link href="/auth/login" className="text-vinho hover:underline font-medium">área da agência</Link>
          </p>
        </div>
      </div>
    </div>
