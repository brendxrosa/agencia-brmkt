...
import { Plus, X, Calendar, Instagram, Video, Image, Layout } from 'lucide-react'
...
export default function KanbanPage() {
  ...
  const [posts, setPosts] = useState<(Post & { clientes: { nome: string; cor: string } | null })[]>([])
  ...
  const [form, setForm] = useState({
    ...
    data_publicacao: '', tema: '', copy: '', legenda: '', direcionamento: ''
  })
  ...
  async function criarPost() {
    ...
    await supabase.from('posts').insert({ ...form, status_interno: 'briefing', status_cliente: 'pendente' })
    setModalAberto(false)
    setForm({ cliente_id: '', titulo: '', tipo: 'reels', data_publicacao: '', tema: '', copy: '', legenda: '', direcionamento: '' })
    carregar()
  }
  ...
  const postsFiltrados = filtroCliente === 'todos'
    ? posts
    : posts.filter(p => p.cliente_id === filtroCliente)

  const postsPorColuna = (status: string) =>
    postsFiltrados.filter(p => p.status_interno === status)
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="flex gap-4 overflow-x-auto pb-6">
          {COLUNAS.map(coluna => (
            <div key={coluna}
              className={cn('w-64 flex-shrink-0 rounded-2xl transition-all', arrastando ? 'bg-creme/80' : 'bg-creme/50')}
              ...
              <div className="p-3 flex items-center justify-between sticky top-0">
                <div className="flex items-center gap-2">
                  <span className={cn('badge text-xs', STATUS_POST_CORES[coluna])}>
                    {STATUS_POST_LABELS[coluna]}
                  </span>
                  <span className="text-xs text-gray-400 font-medium">
                    {postsPorColuna(coluna).length}
                  </span>
                </div>
              </div>
              ...
              <div className="px-3 pb-3 space-y-2 min-h-24">
                {postsPorColuna(coluna).map(post => (
                  <div key={post.id}
                    ...
                    className={cn(
                      'bg-white rounded-xl p-3 shadow-card cursor-grab active:cursor-grabbing group transition-all hover:shadow-card-hover',
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full flex-shrink-0"
                          style={{ backgroundColor: post.clientes?.cor || '#6B0F2A' }} />
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-50 opacity-0 group-hover:opacity-100 transition-all">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {COLUNAS.filter(c => c !== coluna).slice(0, 3).map(c => (
                          <button key={c} onClick={() => moverPost(post.id, c)}
